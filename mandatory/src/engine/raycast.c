...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/03 07:48:36 by llaruell          #+#    #+#             */
/*   Updated: 2023/01/03 13:47:25 by bvernimm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static double	get_perp_distance(t_raycast *ray, t_camera *cam)
{
	double	pos;
 ...
 * @param hit 0
 */
static void	dda(t_game *game, t_raycast *ray, int hit)
{
	while (!hit)
	{
		...
		if (in_map(game->map, ray->map.x, ray->map.y) == -1)
		{
			if (ray->side)
				ray->map.y -= ray->step.y;
			else
		...
		else if (game->map[(int)ray->map.y][(int)ray->map.x] == '1')
			hit = 1;
	}
}
